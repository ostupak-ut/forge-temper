...
export function Settings({ onClose }: { onClose: () => void }) {
  ...
  const createFolder = async () => {
    if (!browse) return
    const name = window.prompt(t('New folder name (created inside the current folder):'))?.trim()
    if (!name) return
    try {
  ...
  const saveFolder = async (dirArg?: string) => {
    ...
    if (!dir) {
      setFolderError(t('Enter an absolute path.'))
      return
    }
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex shrink-0 items-center gap-2 border-b border-border/10 px-4 py-3">
          ...
          <h2 className="text-sm font-semibold text-fg/90">{t('Settings')}</h2>
        ...
        <div className="mb-4 space-y-2 rounded-lg border border-border/15 bg-fg/[0.03] p-3">
          ...
          <label className="block text-[11px] text-fg/55">{t('Language')}</label>
          <select
            value={lang}
          ...
          >
            <option value="uk" className="bg-card">Українська</option>
            <option value="en" className="bg-card">English</option>
          </select>
          <label className="block text-[11px] text-fg/55">{t('App font')}</label>
          <select
            value={font}
          ...
          <label className="block pt-1 text-[11px] text-fg/55">{t('Text size')}</label>
          <select
            value={fontSize}
          ...
          <p className="text-[10px] text-fg/30">{t('Applies instantly, saved in your browser. Toggle light/dark with the ☀/🌙 in the header.')}</p>
        </div>
        ...
        <div className="mb-4 space-y-2 rounded-lg border border-border/15 bg-fg/[0.03] p-3">
          ...
          <p className="text-[11px] text-fg/40">{t('Absolute path; persists across restarts.')}</p>
          ...
          {browse && (
            <div className="rounded-md border border-border/15 bg-card p-2">
              ...
              <div className="max-h-40 overflow-auto">
                {browse.dirs.length === 0 && <p className="px-1 py-1 text-[10px] text-fg/30">{t('(no subfolders)')}</p>}
                {browse.dirs.map((d) => (
                  <button
                    key={d.path}
        ...
        <div className="mb-4 space-y-2 rounded-lg border border-border/15 bg-fg/[0.03] p-3">
          ...
          <label className="flex items-center gap-2 text-[11px] text-fg/70">
            ...
            />
            {t('Inject graph context into every agent')}
          </label>
          <textarea
            value={graphTemplate}
          ...
          <div className="flex items-center justify-end gap-2">
            {graphSaved && <span className="mr-auto text-[10px] text-emerald-300">{t('saved')}</span>}
            <button
              onClick={() => {
            ...
            >
              {savingGraph ? t('Saving…') : t('Save template')}
            </button>
          </div>
        </div>
        ...
        <div className="mb-4 space-y-2 rounded-lg border border-border/15 bg-fg/[0.03] p-3">
          ...
          <p className="text-[11px] text-fg/40">
            {t('Auto-detected by default (Claude Code on PATH; Codex from the IDE extension). Override only if yours live elsewhere.')}
          </p>
          ...
          <div className="flex justify-end">
            ...
            >
              {savingCli ? t('Saving…') : t('Save CLI paths')}
            </button>
          </div>
        </div>

        <h3 className="mb-2 text-xs font-medium text-fg/80">{t('Provider API keys')}</h3>
          ...
          {PROVIDERS.map((p) => (
            <div key={p.key} className="space-y-1">
              <div className="flex items-center gap-2 text-xs text-fg/70">
                {t(p.label)}
                {presence[p.key] ? (
                  <span className="rounded bg-emerald-500/20 px-1.5 text-[10px] text-emerald-300">{t('configured')}</span>
                ) : (
                  <span className="rounded bg-fg/10 px-1.5 text-[10px] text-fg/40">{t('not set')}</span>
                )}
              </div>
              <input
                type="password"
                ...
                className="w-full rounded-md border border-border/15 bg-card px-2 py-1 text-xs text-fg/90 outline-none focus:border-temper"
              />
              <p className="text-[10px] text-fg/25">{t(p.hint)}</p>
            </div>
          ))}
        </div>
        </div>
