...
function PortRow({ port, dir }: { port: Port; dir: 'in' | 'out' }) {
  ...
  return (
    <div
      className={cn(
        'relative flex h-6 items-center text-[10px] text-white/55',
...
function GenericNodeImpl({ id, data, selected }: NodeProps<FtNode>) {
  ...
  return (
    <div
      className={cn(
        'relative w-60 rounded-xl bg-[#121826] shadow-lg ring-2 transition',
        STATUS_RING[status],
        selected && 'outline outline-2 outline-white/40',
      )}
    >
      {/* header */}
      <div
      ...
      >
        ...
        <button
          className="nodrag rounded p-0.5 text-white/30 hover:bg-red-500/20 hover:text-red-300"
          title="Delete node"
          onClick={(e) => {
            e.stopPropagation()
            deleteNode(id)
      ...
      <div className="border-t border-white/5 px-3 py-1.5">
        {run?.tail ? (
          <pre className="max-h-16 overflow-hidden whitespace-pre-wrap break-words text-[10px] leading-tight text-white/45">
            {run.tail.slice(-220)}
          </pre>
        ) : (
          <p className="line-clamp-2 text-[11px] text-white/35">{spec.description}</p>
        )}
        {run?.verdict && (
        ...
        {run?.iteration != null && (
          <span className="mt-1 inline-block rounded bg-white/5 px-1.5 py-0.5 text-[10px] text-white/50">
            iter {run.iteration}
          </span>
        )}
